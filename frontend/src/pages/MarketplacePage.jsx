...
const MarketplacePage = () => {
  ...
  const raw = isDemoMode ? demoMarketplace : (apiData?.strategies || apiData || mockMarketplaceItems);
  const marketplaceItems = Array.isArray(raw) ? raw.map(item => ({
    name: item.name || 'Strategy',
    creator: item.creator || 'AlphaAI',
    category: item.category || item.type || 'Trading Bot',
  ...
  const stats = [
    { label: 'Listed Items', value: String(marketplaceItems.length), change: 'Available', positive: true },
  ...
  const categoryColors = { 'Trading Bot': 'bg-[#7B61FF]/15 text-[#7B61FF]', 'Signal Plugin': 'bg-[#00FF94]/15 text-[#00FF94]', 'Portfolio Tool': 'bg-[#FFB800]/15 text-[#FFB800]', 'On-chain': 'bg-blue-400/15 text-blue-400', Momentum: 'bg-[#7B61FF]/15 text-[#7B61FF]', 'Mean Reversion': 'bg-[#00FF94]/15 text-[#00FF94]' };

  return (
    <div className="min-h-screen pt-24 pb-12 px-4" data-testid="marketplace-page">
      <div className="max-w-7xl mx-auto">
        <PageHeader icon={Store} title="Marketplace" description="Browse and install trading bots, signal plugins, and portfolio tools" testId="marketplace-header" />

        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="mb-8">
          <div className="relative">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-600" />
            <Input placeholder="Search bots, plugins, and tools..." className="bg-[#0A0A0A] border-zinc-800/50 h-12 pl-11 rounded-xl text-sm" data-testid="marketplace-search" />
          </div>
        </motion.div>
                  ...
                  <Card className="bg-[#0A0A0A] border-zinc-800/50 hover:border-zinc-700/50 transition-colors" data-testid={`marketplace-item-${i}`}>
                    <CardContent className="p-5">
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <Badge className={categoryColors[item.category] || 'bg-zinc-600/15 text-zinc-400'}>{item.category}</Badge>
                      </div>
                      <div className="flex items-center gap-4 mb-4 text-xs text-zinc-500">
                        <span className="flex items-center gap-1"><Star className="w-3 h-3 text-[#FFB800]" /> {item.rating}</span>
                        <span className="flex items-center gap-1"><Users className="w-3 h-3" /> {(item.users || 0).toLocaleString()} users</span>
                      </div>
