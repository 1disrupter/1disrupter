...
import {
  PodiumSkeleton, LeaderboardRowSkeleton, StrategyDetailSkeleton,
} from "../components/SkeletonLoaders";
import { prefetchStrategy, getCachedStrategy } from "../lib/smartPrefetch";
...
const LeaderboardPage = () => {
  ...
  const toggleCompact = () => {
    setCompact(p => {
      const next = !p;
      try { localStorage.setItem("alphaai_compact", String(next)); } catch {}
      return next;
    });
  };

  const loadFollowedIds = useCallback(async () => {
    if (isDemoMode) { setFollowedIds(new Set(["demo-1", "demo-2"])); return; }
    if (!token) return;
    try {
  ...
  const toggleFollow = async (stratId) => {
    if (!isOnline) { toast.info("You're offline — follow/unfollow disabled"); return; }
    if (isDemoMode) {
      setFollowedIds(prev => {
    ...
    try {
      ...
      if (e.response?.status === 403) setUpgradeOpen(true);
      else toast.error("Failed to update follow");
    }
  };

  const handleSort = (colApiKey) => {
    if (sortBy === colApiKey) setSortOrder(prev => prev === "desc" ? "asc" : "desc");
    else { setSortBy(colApiKey); setSortOrder(colApiKey === "max_drawdown" ? "asc" : "desc"); }
  };
  ...
  return (
    <PullToRefresh onRefresh={loadStrategies} disabled={!("ontouchstart" in window)}>
      <div className="min-h-screen pt-24 pb-12 px-4" data-testid="leaderboard-page">
        <div className="max-w-6xl mx-auto">
          {/* Offline Banner */}
          {!isOnline && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-4 p-3 rounded-xl bg-zinc-800/50 border border-zinc-700 flex items-center gap-2 text-sm text-zinc-400" data-testid="offline-banner">
              <WifiOff className="w-4 h-4" /> You're offline — showing cached data
            </motion.div>
          )}
          ...
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button onClick={toggleCompact} className={`hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] border transition-colors ${compact ? "bg-[#7B61FF]/10 border-[#7B61FF]/30 text-[#7B61FF]" : "border-zinc-800 text-zinc-500 hover:text-zinc-300"}`} data-testid="compact-toggle">
                  <BarChart3 className="w-3 h-3" /> {compact ? "Compact" : "Default"}
                </button>
          ...
          {loading && strategies.length === 0 ? (
            ...
            <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="grid grid-cols-3 gap-4 mb-8">
              {[1, 0, 2].map((idx) => {
                ...
                return (
                  <Card key={idx} className={`bg-[#0A0A0A] ${medal.color} ${idx === 0 ? "md:-mt-4" : ""} cursor-pointer transition-all hover:scale-[1.02]`} data-testid={`podium-${idx}`} onClick={() => openDetail(s)} onMouseEnter={() => prefetchStrategy(s.id)}>
                    ...
                    <table className="w-full" data-testid="strategy-table">
                      <thead>
                        <tr className="border-b border-zinc-800/50">
                          <th className={`${cellPad} text-left text-xs font-medium text-zinc-500 uppercase tracking-wider w-12`}>#</th>
                          <th className={`${cellPad} text-left text-xs font-medium text-zinc-500 uppercase tracking-wider`}>Strategy</th>
                          <th className={`${cellPad} text-left text-xs font-medium text-zinc-500 uppercase tracking-wider`}>Type</th>
                          <th className={`${cellPad} text-left text-xs font-medium text-zinc-500 uppercase tracking-wider`}>Asset</th>
                          {SORT_COLS.map(col => (
                            <th key={col.key} className={`${cellPad} text-right text-xs font-medium text-zinc-500 uppercase tracking-wider cursor-pointer select-none hover:text-zinc-300 transition-colors`} onClick={() => handleSort(col.apiKey)} data-testid={`sort-${col.apiKey}`}>
                          ...
                          <th className={`${cellPad} text-right text-xs font-medium text-zinc-500 uppercase tracking-wider`}>Trades</th>
                          <th className={`${cellPad} text-center text-xs font-medium text-zinc-500 uppercase tracking-wider w-28`}>Follow</th>
                          <th className={`${cellPad} text-center text-xs font-medium text-zinc-500 uppercase tracking-wider w-20`}></th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-zinc-800/30">
                        {strategies.map((s, i) => (
                          <tr key={s.id || i} className="hover:bg-white/[0.02] transition-colors" data-testid={`leaderboard-row-${i}`} onMouseEnter={() => prefetchStrategy(s.id)} onTouchStart={() => prefetchStrategy(s.id)}>
                            ...
                            <td className={`${cellPad} text-sm font-medium text-zinc-200 max-w-[200px] truncate`}>{s.name}</td>
                            <td className={cellPad}><Badge className={typeColors[s.type] || "bg-zinc-700 text-zinc-400"}>{s.type?.replace("_", " ")}</Badge></td>
                            <td className={`${cellPad} text-xs font-mono text-zinc-400`}>{s.asset}</td>
                            <td className={`${cellPad} text-right text-sm font-mono font-bold text-white`}>{s.metrics?.sharpe_ratio}</td>
                            <td className={`${cellPad} text-right text-sm font-mono ${(s.metrics?.total_return || 0) >= 0 ? "text-[#00FF94]" : "text-red-400"}`}>{s.metrics?.total_return}%</td>
                            <td className={`${cellPad} text-right text-sm font-mono text-red-400`}>{s.metrics?.max_drawdown}%</td>
                            <td className={`${cellPad} text-right text-sm font-mono text-zinc-300`}>{s.metrics?.win_rate}%</td>
                            <td className={`${cellPad} text-right text-xs font-mono text-zinc-500`}>{s.metrics?.total_trades || "--"}</td>
                            <td className={`${cellPad} text-center`}>
                              <Button size="sm" variant={followedIds.has(s.id) ? "default" : "outline"} onClick={(e) => { e.stopPropagation(); toggleFollow(s.id); }} disabled={!isOnline} className={`rounded-full text-[10px] h-7 px-3 ${followedIds.has(s.id) ? "bg-red-500/15 text-red-400 border-red-500/30 hover:bg-red-500/25" : "border-zinc-800 hover:border-[#7B61FF]/50"} ${!isOnline ? "opacity-50 cursor-not-allowed" : ""}`} data-testid={`follow-${i}`}>
          ...
          <AnimatePresence>
            {(detailModal || detailLoading) && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => { setDetailModal(null); setDetailLoading(false); }}>
                        ...
                        <CardContent className="p-5 space-y-5">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            {[
                              ...
                              <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="p-3 rounded-lg bg-[#050505] border border-zinc-800/30 text-center">
                            ...
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
                              ...
                              <div className="h-32 flex items-end gap-[2px]">
                                {detailModal.metrics.equity_curve.map((pt, i) => {
                                  ...
                                  return (<motion.div key={i} initial={{ scaleY: 0 }} animate={{ scaleY: 1 }} transition={{ delay: i * 0.01, duration: 0.3 }} className="flex-1 rounded-t bg-[#00FF94]/50 hover:bg-[#00FF94]/70 transition-colors origin-bottom" style={{ height: `${Math.max(pct, 3)}%` }} title={`$${Math.round(pt.value).toLocaleString()}`} />);
                                })}
                              </div>
                            </motion.div>
                          )}
                          ...
                          <div className="pt-2 border-t border-zinc-800/50">
                            <Button onClick={() => { toggleFollow(detailModal.id); setDetailModal(null); }} disabled={!isOnline} className={`w-full rounded-full text-xs ${followedIds.has(detailModal.id) ? "bg-red-500/15 text-red-400 border border-red-500/30 hover:bg-red-500/25" : "bg-[#7B61FF] hover:bg-[#7B61FF]/80 text-white"} ${!isOnline ? "opacity-50 cursor-not-allowed" : ""}`} data-testid="detail-follow-btn">
