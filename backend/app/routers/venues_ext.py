...
def _serialize(v: Venue, vibe: Vibe | None, sig: VenueSignals | None,
               user_lat: Optional[float], user_lng: Optional[float]) -> dict:
    out = {
        "id": v.id,
        "name": v.name,
...
def venues_list(
    ...
    items = [_serialize(v, vibe, signals.get(v.id), user_lat, user_lng) for v, vibe in rows]
...
def intel_score(
    ...
    return _serialize(v, vibe, sig, user_lat, user_lng)
